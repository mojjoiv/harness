...
const HEALTH_META: Record<
  string,
  { emoji: string; label: string; tone: 'neutral' | 'green' | 'red' | 'blue' }
> = {
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-border bg-panel shadow-2xl"
      >
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-border bg-panel px-6 py-5">
          ...
          <button
            ...
            className="rounded-xl p-2 text-muted transition hover:bg-panelAlt hover:text-ink"
          >
        ...
        <div className="space-y-4 p-6">
          ...
          <Panel className="p-5">
            ...
            <DetailRow
              label="Latency"
              value={credential.verificationLatencyMs != null ? `${credential.verificationLatencyMs}ms` : '—'}
            />
            <DetailRow
              label="Last verified"
              value={credential.lastVerifiedAt ? dateTime(credential.lastVerifiedAt) : 'Never'}
            />
            {credential.verificationWarnings.length > 0 ? (
              <div className="mt-3 rounded-2xl bg-panelAlt p-3 text-sm text-muted">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  {credential.verificationWarnings.map((warning) => <li key={warning}>{warning}</li>)}
                </ul>
              </div>
            ) : null}
            {credential.verificationErrors.length > 0 ? (
              <div className="mt-3 rounded-2xl bg-rose-50 p-3 text-sm text-rose-700">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  {credential.verificationErrors.map((error) => <li key={error}>{error}</li>)}
                </ul>
              </div>
            ) : null}
            {credential.lastVerificationError ? (
              <div className="mt-3 rounded-2xl bg-rose-50 p-3 text-sm text-rose-700">
                Last verification error: {credential.lastVerificationError}
              </div>
            ) : null}
            {credential.failedVerificationCount > 0 ? (
              <div className="mt-3 text-xs text-muted">
                Failed verification attempts: {credential.failedVerificationCount}
              </div>
            ) : null}
          </Panel>
          ...
          <Panel className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted">{dateTime(credential.updatedAt)}</p>
              </div>
            </div>
          </Panel>
            ...
            {!isRevoked ? (
              <>
                ...
                {!credential.isDefault ? (
                  <Button variant="secondary" disabled={busy} onClick={onSetDefault}>
                    Set Default
                  </Button>
                ) : null}
