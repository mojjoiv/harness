...
export default function PayoutsPage() {
  ...
  const load = async (nextPage = page, nextFilters = filters) => {
    ...
    try {
      ...
      setItems(response.data.data); setMeta(response.data);
    ...
    } catch (err) { setError(err instanceof ApiError ? err.message : 'Unable to load payouts.'); }
  ...
  const selectPayout = async (id: string) => {
    ...
    try { const response = await api.get<Payout>(`/payouts/${id}`); setSelected(response.data); }
    catch (err) { setActionError(err instanceof ApiError ? err.message : 'Unable to load payout details.'); }
    finally { setDetailLoading(false); }
  };

  const createPayout = async () => {
    const amountCents = Math.round(Number(form.amount) * 100);
    if (!Number.isFinite(amountCents) || amountCents < 1 || !form.currency || !form.recipientType) { setActionError('Enter a valid amount, currency, and recipient type.'); return; }
    if (form.provider === 'MPESA' && !form.recipientPhone.trim()) { setActionError('M-Pesa payouts require a recipient phone number.'); return; }
    setActionLoading(true); setActionError(''); setActionMessage('');
    try {
      const key = `dashboard-payout-${crypto.randomUUID()}`;
      const response = await api.post<Payout>('/payouts', { amountCents, currency: form.currency, provider: form.provider, environment: form.environment, recipientType: form.recipientType, recipientPhone: form.recipientPhone || undefined, recipientName: form.recipientName || undefined }, { headers: { 'Idempotency-Key': key } });
      ...
      await load(1, filters); setPage(1);
    } catch (err) { setActionError(err instanceof ApiError ? err.message : 'Unable to create payout.'); }
  ...
  const execute = async () => {
    if (!selected) return;
    setActionLoading(true); setActionError(''); setActionMessage('');
    try { const response = await api.post<Payout>(`/payouts/${selected.id}/execute`, {}); setSelected(response.data); setActionMessage(`Payout is now ${response.data.status}.`); await load(page, filters); }
    catch (err) { setActionError(err instanceof ApiError ? err.message : 'Unable to execute payout.'); }
    finally { setActionLoading(false); }
  };

  const stats = useMemo(() => report || { totalCount: 0, totalVolumeCents: 0, successfulCount: 0, successfulVolumeCents: 0, failedCount: 0, failedVolumeCents: 0, pendingCount: 0, pendingVolumeCents: 0, successRate: 0 }, [report]);

  return <div className="space-y-6">
    <SectionTitle title="Payouts" description="Create, monitor, and execute merchant payouts across configured providers." action={<Button variant="secondary" onClick={() => void load()} disabled={loading}>{loading ? 'Refreshing…' : 'Refresh'}</Button>} />
    ...
    <Panel className="p-6">
      <div className="mb-5"><h2 className="text-lg font-semibold">Create payout</h2><p className="mt-1 text-sm text-muted">An idempotency key is generated for each dashboard submission.</p></div>
