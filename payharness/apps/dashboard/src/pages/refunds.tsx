...
type RefundDetail = RefundRecord & {
...
export default function RefundsPage() {
  ...
  const load = async (currentPage = page, currentFilters = filters) => {
    ...
    try {
      ...
      const response = await api.get<RefundRecord[]>(`/transactions?${params.toString()}`);
  ...
  const loadDetail = async (id: string) => {
    ...
    try {
      const response = await api.get<RefundDetail>(`/transactions/${id}`);
  ...
  const submitRefund = async () => {
    if (!paymentId.trim()) {
      setSubmitError('Payment ID is required.');
      return;
    }
    ...
    if (amount.trim() && (!Number.isFinite(amountCents) || amountCents <= 0)) {
      setSubmitError('Refund amount must be a positive amount.');
      return;
    }
    ...
    try {
      const response = await api.post<RefundResponse>(
        `/payments/${encodeURIComponent(paymentId.trim())}/refund`,
        amountCents === undefined ? {} : { amountCents },
      );
      setSubmitMessage(
        `${response.data.status.replaceAll('_', ' ')}: ${money(response.data.amountCents, response.data.currency)} refunded.`,
      );
      setPaymentId('');
      setAmount('');
  ...
  const rows = items.map((refund) => [
    <button
      ...
      onClick={() => void loadDetail(refund.id)}
    >
      {compactId(refund.id)}
    </button>,
    <button
      key={`${refund.id}-payment`}
      type="button"
      className="text-brand hover:underline"
      onClick={() => setPaymentId(refund.paymentId)}
    >
      {compactId(refund.paymentId)}
    </button>,
    ...
    money(refund.amountCents, refund.currency),
    refund.reference ? compactId(refund.reference) : '—',
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        title="Refunds"
        description="Review merchant refunds and issue full or partial refunds for eligible payments."
        action={
      ...
      <Panel className="p-6">
        <div className="mb-5">
          <h2 className="text-lg font-semibold text-ink">Issue a refund</h2>
          <p className="mt-1 text-sm text-muted">Leave the amount blank for a full refund. Enter an amount for a partial refund.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-[1fr_220px_auto] md:items-end">
          ...
          <FieldRow label="Amount (optional)">
            <Input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Full refund if blank" />
          </FieldRow>
          <Button type="button" onClick={() => void submitRefund()} disabled={submitting}>
      ...
      {loading && !items.length ? (
        <Panel className="p-6 text-sm text-muted">Loading refunds…</Panel>
      ) : (
        <SimpleTable
          headers={['Refund', 'Payment', 'Provider', 'Status', 'Amount', 'Reference', 'Created']}
          rows={rows}
          emptyText="No refunds match the selected filters."
        />
      )}
      ...
      {selectedId ? (
        <Panel className="p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-ink">Refund details</h2>
          ...
          {detail ? (
            <div className="mt-6 space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Panel className="p-4">
                  <div className="text-sm font-medium text-ink">Refund</div>
                  <dl className="mt-4 space-y-3 text-sm">
                    <div className="flex justify-between gap-4"><dt className="text-muted">Refund ID</dt><dd className="max-w-[60%] break-all text-right">{detail.id}</dd></div>
                    ...
                    <div className="flex justify-between gap-4"><dt className="text-muted">Created</dt><dd className="text-right">{dateTime(detail.createdAt)}</dd></div>
                  </dl>
                </Panel>
                <Panel className="p-4">
                  <div className="text-sm font-medium text-ink">Original payment</div>
                  <dl className="mt-4 space-y-3 text-sm">
                    <div className="flex justify-between gap-4"><dt className="text-muted">Status</dt><dd>{detail.payment?.status || '—'}</dd></div>
                    <div className="flex justify-between gap-4"><dt className="text-muted">Amount</dt><dd>{detail.payment ? money(detail.payment.amountCents, detail.payment.currency) : '—'}</dd></div>
                    <div className="flex justify-between gap-4"><dt className="text-muted">Provider reference</dt><dd className="max-w-[60%] break-all text-right">{detail.payment?.providerReference || '—'}</dd></div>
                  </dl>
                </Panel>
              </div>
            </div>
