...
describe('PayoutExecutionService', () => {
  it('moves a pending payout to succeeded after provider execution', async () => {
    const prisma = {
      $queryRaw: jest
        ...
        .mockResolvedValueOnce([{ ...payout, status: 'SUCCEEDED', providerReference: 'mpesa-123' }]),
  ...
  it('marks a payout failed when provider execution throws', async () => {
    ...
    expect(prisma.$executeRaw).toHaveBeenCalledTimes(2);
    const failedSql = prisma.$executeRaw.mock.calls[1][0];
    expect(failedSql.values).toContain('provider unavailable');
  });
